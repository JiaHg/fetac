...
# 		return pil_loader(path)
"""
if loading dataset error, try to change the default loader
"""
def default_loader(path):
	with open(path, 'rb') as f:
		img = io.imread(path)
...
class Imagefolder_normal(object):
	"""
	...
	def __init__(self, data_dir="", mode="train", image_size=84, data_name="miniImageNet",
				 ...
				 episode_num=10, way_num=5, shot_num=5, query_num=5, total_file=1000, test_start=10, test_end= 20):

		super(Imagefolder_normal, self).__init__()

		class_img_dict = {}

		image_datasets = datasets.ImageFolder(data_dir)
		for i in range(total_file):
			(path, img_class) = image_datasets.samples[i]
			if img_class in class_img_dict:
				class_img_dict[img_class].append(path)
			else:
		...
		e = 0
		if mode == "train":
			l = [i for i in range(test_start)] # auxiliary dataset

			class_img_dict = dict([(key, class_img_dict[key]) for key in l])
		...
		if mode == "test":
			...
			l = [i for i in range(test_start, test_end)] # auxiliary_dataset
			# print(l)
			class_img_dict = dict([(key, class_img_dict[key]) for key in l])
